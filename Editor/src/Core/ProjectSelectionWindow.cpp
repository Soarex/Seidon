...
namespace Seidon
{
	...
	Project* ProjectSelectionWindow::CreateNewProject()
	{
		...
		p->name = GetNameFromPath(filepath);
		...
		editor.GetResourceManager()->RegisterAsset(scene, filepath + "\\Assets\\Scenes\\Default Scene.sdscene");

		p->loadedScene = scene;

		p->AddEditorSystem<RenderSystem>();
		p->AddEditorSystem<EditorCameraControlSystem>();
